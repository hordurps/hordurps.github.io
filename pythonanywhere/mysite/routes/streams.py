...
for mesh_filename in f:
    ...
    child = dash_vtk.GeometryRepresentation(
        ...
        children=[dash_vtk.Mesh(id=f"{mesh_name}-mesh", state=mesh)],
    )
    #points = mesh.points.ravel()
    #polys = vtk_to_numpy(mesh.GetPolys().GetData())
    #arrays = list(mesh.point_data)
    ...
    mesh_ids[mesh_name] = f"{mesh_name}-mesh"
...
for stl in stls:
    ...
    child = dash_vtk.GeometryRepresentation(
        ...
        actor={"visibility" : 1},
        mapper={"scalarVisibility" : False},
        children=[dash_vtk.Mesh(id=f"{stl_name}-mesh", state=stl_mesh)]
    )
    buildings_rep.append(child)
    buildings_mesh_ids.append(f"{stl_name}-mesh")
...
@app.callback(Output("vtk-view-container", "children"),
    [Input("toggle-stls", "value"), Input("dropdown-meshes", "value"), Input("dropdown-array-preset", "value")]
    )
...
def update_scene(stls, selected_mesh_name, selected_array_name, cubeAxes, showStreams):
    ...
    geom_visibility = []
    if triggered and "toggle-stls" in triggered[0]["prop_id"]:
        geom_visibility = [
            {"visibility" : 1}
        if part.id.replace("-rep", "") in triggered[0]["value"]
        else {"visibility" : 0}
    ...
    if triggered and "dropdown-meshes" in triggered[0]["prop_id"]:
    ...
    if triggered and "dropdown-array-preset" in triggered[0]["prop_id"]:
